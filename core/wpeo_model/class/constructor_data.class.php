...
class constructor_data_class {
	 ...
	 *
	 * @param Object $current_object Définition complète de l'objet à enregistrer / Complete object definition to save
	 ...
	 */
	public function save_meta_data( $current_object, $function, $meta_key ) {
		/** Read the object model option */
		$array_option = $current_object->get_array_option();

		foreach ( $current_object->option as $field_name => $field ) {
			if( !isset( $array_option[ $field_name ]['type'] ) || is_array( $array_option[ $field_name ]['type'] ) ) {
				foreach( $field as $sub_field_name => $sub_field ) {
					...
					if ( isset( $array_option[ $field_name ][ $sub_field_name ][ 'field_type' ] ) ) {
						if ( 'meta' == $array_option[ $field_name ][ $sub_field_name ][ 'field_type' ] ) {
							...
							call_user_func( $function, $current_object->id, $array_option[ $field_name ][ $sub_field_name ][ 'field' ] , $current_object->option[ $field_name ][ $sub_field_name ] );
							unset( $current_object->option[ $field_name ][ $sub_field_name ] );
						}
				...
				if ( 'meta' == $array_option[ $field_name ][ 'field_type' ] ) {
					/** Update a single meta for specific field */
					call_user_func( $function, $current_object->id, $array_option[ $field_name ][ 'field' ], $current_object->option[ $field_name ] );
					unset( $current_object->option[ $field_name ] );
				}
		...
		$encoded_object = json_encode( $current_object->option );
		...
		call_user_func( $function, $current_object->id, $meta_key, $encoded_object );
	}
