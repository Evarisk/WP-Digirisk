...
<ul class="module-list" >
	<li class="module-list-header" >
		<!--
		<span class="module-cb" ><?php _e( 'Activ', $this->text_domain ); ?></span>
		 -->
		<span class="module-name" ><?php _e( 'Module name', $this->text_domain ); ?></span>
		<span class="module-name" ><?php _e( 'Module description', $this->text_domain ); ?></span>
		<span class="module-info" ><?php _e( 'Module info', $this->text_domain ); ?></span>
	</li>
...
<?php foreach ( $parent_folder_content as $folder ) : ?>
	...
	<li class="<?php echo ( $i == 0 ? "odd" : "even" ); ?>" >
		...
		<span class="module-description" >
			...
			<?php echo !empty( $module_datas ) && !empty( $module_datas[ 'Version' ] ) ? $module_datas[ 'Version' ] : __( 'Unknown version', $this->text_domain ); ?> |
			<?php echo !empty( $module_datas ) && !empty( $module_datas[ 'Author' ] ) ? $module_datas[ 'Author' ] : __( 'Unknown author', $this->text_domain ); ?>
		...
		<span class="module-info module-info-<?php echo $folder; ?>" >
			<?php
				if ( 'auto' != $module_option[ $folder ][ 'author_on' ] ) :
					...
					$author = __( 'automatic activation', $this->text_domain );
				endif;
				printf( __( 'Last activation made on %1$s by %2$s', $this->text_domain ), mysql2date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $module_option[ $folder ][ 'date_on' ], true ), $author);
			?>
			...
			<?php
				...
				$author = $user->display_name;
				printf( __( 'Last deactivation made on %1$s by %2$s', $this->text_domain ), mysql2date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $module_option[ $folder ][ 'date_off' ], true ), $author);
			?>
			<?php endif; ?>

			<?php apply_filters( 'wpeo_addon_extra_info', $folder ); ?>
		</span>
	</li>
	<?php endif; ?>
	<?php
