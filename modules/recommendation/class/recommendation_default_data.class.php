...
class recommendation_default_data_class extends singleton_util {
	...
	*/
	public function create() {
		$file_content = file_get_contents( PLUGIN_PATH . config_util::$init['recommendation']->path . 'asset/json/default.json' );
	...
	private function create_recommendation_category( $json_recommendation_category ) {
		...
		$file_id = file_util::g()->move_file_and_attach( PLUGIN_PATH . '/core/assets/images/preconisations/' . $json_recommendation_category->name_thumbnail, 0 );
	...
	private function create_recommendation( $recommendation_category, $json_recommandation ) {
		...
		if ( !is_wp_error( $recommandation ) ) {
			$file_id = file_util::g()->move_file_and_attach( PLUGIN_PATH . '/core/assets/images/preconisations/' . $json_recommandation->name_thumbnail, 0 );
