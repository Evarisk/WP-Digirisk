...
class wpdigi_transferdata_society_ctr_01 extends TransferData_common_controller {
	...
	function transfer_groupement( $evarisk_element, $element_type, $wp_element_id ) {
		...
		$group_mdl = $wpdigi_group_ctr->show( $wp_element_id );
		...
		$meta = array(
			...
			'user_info' => array(
				...
				'affected_id' => $this->transfer_users( $evarisk_element->id, $element_type, null, $wpdigi_group_ctr->get_post_type(), $wp_element_id ),
			),
			'contact' => array(
		...
		$wpdigi_group_ctr->update( $group_mdl );
	}
	...
	function transfer_unite( $element, $element_type, $wp_element_id ) {
		...
		/**	Get the element created for new data transfer	*/
		$workunit_mdl = $wpdigi_workunit_ctr->show( $wp_element_id );
		...
		$meta = array(
			...
			'user_info' => array(
				...
				'affected_id' => $this->transfer_users( $element->id, $element_type, null, $wpdigi_workunit_ctr->get_post_type(), $wp_element_id ),
			),
			'contact' => array(
		...
		$wpdigi_workunit_ctr->update( $workunit_mdl );
	}
	...
	function transfer_risk( $old_element_id, $old_element_type, $new_element_id ) {
		global $wpdb, $wpdigi_risk_ctr, $wpdigi_risk_evaluation_ctr,
					$wpdigi_danger_category_ctr, $wpdigi_danger_ctr, $wpdigi_evaluation_method_controller, $wpdigi_risk_evaluation_comment_ctr, $wpdigi_evaluation_method_variable_controller;
		...
		if ( !empty( $element_risks ) ) {
			foreach ( $element_risks as $old_risk_id => $old_risk_evaluation_list ) {
				...
				$risk_definition = array(
					...
					'title'						=> sprintf( __( '%1$s for %3$s - %2$s', 'wpdigi-i18n' ), $danger->nom, $old_element->nom, $element_prefix ),
					'slug'						=> null,
					'content'					=> null,
					'status'					=> $status,
					'link'						=> null,
					'type'						=> $wpdigi_risk_ctr->get_post_type(),
				...
				$wp_risk = $wpdigi_risk_ctr->create( $risk_definition );
				if ( !empty( $wp_risk->id ) ) {
					...
					/**	Log creation	*/
					wpeologs_ctr::log_datas_in_files( 'digirisk-datas-tranfert-risk', array( 'object_id' => $old_risk_id, 'message' => sprintf( __( 'Risk have been successfully transfered to wordpress system id. %d', 'wpdigi-i18n' ), $wp_risk->id ), ), 0 );
					...
					if ( !empty( $new_danger_category ) && !empty( $new_danger_category->term_id )  ) {
						...
						/**	Log creation	*/
						wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-risk-association', array( 'object_id' => $wp_risk->id, 'message' => sprintf( __( 'Danger category %s - %d have been associated to risk', 'wpdigi-i18n' ), $wpdigi_danger_category_ctr->get_taxonomy(), (int)$new_danger_category->term_id ), ), 0 );
						...
						/**	Log creation	*/
						wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-risk-association', array( 'object_id' => $wp_risk->id, 'message' => sprintf( __( 'Danger category to associate have not been found. %d', 'wpdigi-i18n' ), json_encode( $new_danger_category )), ), 2 );
					}
					...
					if ( !empty( $new_danger_id ) ) {
						...
						/**	Log creation	*/
						wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-risk-association', array( 'object_id' => $wp_risk->id, 'message' => sprintf( __( 'Danger %s - %d have been associated to risk', 'wpdigi-i18n' ), $wpdigi_danger_ctr->get_taxonomy(), (int)$new_danger_id ), ), 0 );
					}
					else {
						/**	Log creation	*/
						wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-risk-association', array( 'object_id' => $wp_risk->id, 'message' => __( 'Danger to associate have not been found.', 'wpdigi-i18n'), ), 2 );
					}
					...
					if ( !empty( $new_method_id ) ) {
						...
						/**	Log creation	*/
						wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-risk-association', array( 'object_id' => $wp_risk->id, 'message' => sprintf( __( 'Method %d have been associated to risk', 'wpdigi-i18n' ), (int)$new_method_id ), ), 0 );
					}
					else {
						/**	Log creation	*/
						wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-risk-association', array( 'object_id' => $wp_risk->id, 'message' => __( 'Method to associate have not been found.', 'wpdigi-i18n'), ), 2 );
					}
					...
					foreach ( $old_risk_evaluation_list as $old_risk_evaluation_id => $old_risk_evaluation ) {
						...
						$wp_risk_evaluation = $wpdigi_risk_evaluation_ctr->create( $risk_evaluation_definition );
						if ( !empty( $wp_risk_evaluation->id ) ) {
							$wp_risk->option[ 'associated_evaluation' ][] = $wp_risk_evaluation->id;

							/**	Log creation	*/
							wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-risk-association', array( 'object_id' => $wp_risk->id, 'message' => sprintf( __( 'Evaluation %d have been associated to risk under %s', 'wpdigi-i18n' ), (int)$risk_evaluation_definition[ 'option' ][ 'unique_key' ], $wp_risk_evaluation->id ), ), 0 );
							...
							if ( !empty( $comment_list ) ) {
								foreach ( $comment_list as $comment ) {
									...
									$risk_evaluation_comment_definition = array(
										'id'										=> null,
										'post_id'								=> $wp_risk->id,
										'date'									=> $comment->date,
									...
									if ( !empty( $wp_risk_evaluation_comment->id ) ) {
										/**	Log creation	*/
										wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-risk-association', array( 'object_id' => $wp_risk->id, 'message' => sprintf( __( 'Evaluation comment %d have been associated to risk under %s', 'wpdigi-i18n' ), (int)$comment->id, $wp_risk_evaluation_comment->id ), ), 0 );
									}
								}
							}
							...
							/**	Log creation	*/
							wpeologs_ctr::log_datas_in_files( 'digirisk-datas-tranfert-risk', array( 'object_id' => $old_risk_id, 'message' => sprintf( __( 'Unable to transfer risk evaluation to wordpress system. Error: %s', 'wpdigi-i18n' ), json_encode( $wp_risk_evaluation ) ), ), 2 );
						}
					}

					/** Mise à jour du risk avec les données ajoutées / Update the risk with new datas */
					$wpdigi_risk_ctr->update( $wp_risk );
				}
				else {
					/**	Log creation	*/
					wpeologs_ctr::log_datas_in_files( 'digirisk-datas-tranfert-risk', array( 'object_id' => $old_risk_id, 'message' => sprintf( __( 'Unable to transfer risk to wordpress system. %d', 'wpdigi-i18n' ), json_encode( $wp_risk ) ), ), 2 );
				}
			}
		}
	...
	function transfer_recommendation( $old_element_id, $old_element_type, $new_element_id ) {
		global $wpdb, $digi_recommendation_controller;
		...
		foreach ( $associated_recommendations as $recommendation ) {
			...
			if ( !empty( $new_recommendation_id ) ) {
				wp_set_object_terms( $new_element_id, (int)$new_recommendation_id, $digi_recommendation_controller->get_taxonomy(), true );
				...
				$associated_recommendation[ (int)$new_recommendation_id ][] = array(
					...
					'unique_identifier'	=> $digi_recommendation_controller->element_prefix . $recommendation->id,
	...
	function transfer_addresse( $address_id, $new_element_id ) {
		global $wpdb, $wpdigi_address_ctr;
		$addresses = array();
		...
		$address_definition = array(
			...
			'author_email'					=> null,
			'content'								=> null,
			'type'									=> $wpdigi_address_ctr->get_type(),
			'parent_id'							=> null,
			'user_id'								=> null,
			'author_id'							=> null,
			'status'								=> -34070,
		...
		$wp_address = $wpdigi_address_ctr->create( $address_definition );
