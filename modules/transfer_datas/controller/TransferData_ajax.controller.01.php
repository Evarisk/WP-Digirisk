	...
	function launch_transfer() {
		...
		if ( !empty( $element_type ) ) {
			...
			if ( 'config_components' != $sub_action ) {
				...
				$transfer_progression = $this->get_transfer_progression( $element_type, $sub_element_type );
				...
				if ( $element_done && $document_done ) {
					...
					update_option( $digi_recommendation_controller->last_affectation_index_key, $wpdb->get_var( $query ) );
			...
			switch ( $sub_action ) {
				case 'config_components':
					...
					$config_components_transfert = new TransferData_components_controller();
					...
					if ( empty( $digirisk_transfert_options ) || empty( $digirisk_transfert_options[ 'danger' ] ) || ( !empty( $digirisk_transfert_options[ 'danger' ] ) && !empty( $digirisk_transfert_options[ 'danger' ][ 'state' ] ) && ( 'complete' != $digirisk_transfert_options[ 'danger' ][ 'state' ] ) ) ) {
						$eva_danger_tranfer_result = $config_components_transfert->transfer_danger( $digirisk_transfert_options );
					...
					if ( empty( $digirisk_transfert_options ) || empty( $digirisk_transfert_options[ 'evaluation_method' ] ) || ( !empty( $digirisk_transfert_options[ 'evaluation_method' ] ) && !empty( $digirisk_transfert_options[ 'evaluation_method' ][ 'state' ] ) && ( 'complete' != $digirisk_transfert_options[ 'evaluation_method' ][ 'state' ] ) ) ) {
						$eva_evaluation_method_tranfer_result = $config_components_transfert->transfer_evaluation_method( $digirisk_transfert_options );
					...
					if ( empty( $digirisk_transfert_options ) || empty( $digirisk_transfert_options[ 'recommendation' ] ) || ( !empty( $digirisk_transfert_options[ 'recommendation' ] ) && !empty( $digirisk_transfert_options[ 'recommendation' ][ 'state' ] ) && ( 'complete' != $digirisk_transfert_options[ 'recommendation' ][ 'state' ] ) ) ) {
						$eva_recommendation_tranfer_result = $config_components_transfert->transfer_recommendation( $digirisk_transfert_options );
					...
					if ( empty( $digirisk_transfert_options ) || empty( $digirisk_transfert_options[ 'document_model' ] ) || ( !empty( $digirisk_transfert_options[ 'document_model' ] ) && !empty( $digirisk_transfert_options[ 'document_model' ][ 'state' ] ) && ( 'complete' != $digirisk_transfert_options[ 'document_model' ][ 'state' ] ) ) ) {
						$eva_models_tranfer_result = $config_components_transfert->transfer_document_model( $digirisk_transfert_options );
	...
	function element_transfer( $element_type, $sub_element_type, $nb_element_to_transfert, $transfered_element ) {
		...
		if ( count( $transfered_element[ $element_type ] ) < $nb_element_to_transfert->main_element_nb ) {
			...
			foreach ( $first_level_elements as $element ) {
				if ( !empty( $element ) ) {
					$new_element_id = $common_data_transfer->transfer( $element_type, $element );
			...
			if ( !empty( $children ) ) {
				foreach ( $children as $child ) {
					...
					$new_children_id = $common_data_transfer->transfer( $sub_element_type, $child, $element_parent_id );
	...
	function document_transfer( $element_type, $sub_element_type, $nb_element_to_transfert, $transfered_element ) {
		...
		if ( !empty( $pictures ) ) {
			foreach ( $pictures as $picture ) {
				...
				$document_id = $common_data_transfer->transfer_document( $picture, $new_element_id, 'picture' );
				if ( ( null !== $document_id ) && !is_wp_error( $document_id ) ) {
					...
					switch ( $picture->tableElement ) {
						case TABLE_UNITE_TRAVAIL:
								...
								$elt = $wpdigi_workunit_ctr->show( $new_element_id );
								$elt->option[ 'associated_document_id' ][ 'image' ][] = $document_id;
								$wpdigi_workunit_ctr->update( $elt );
							break;
						case TABLE_GROUPEMENT:
								global $wpdigi_group_ctr;
								$elt = $wpdigi_group_ctr->show( $new_element_id );
								$elt->option[ 'associated_document_id' ][ 'image' ][] = $document_id;
								$wpdigi_group_ctr->update( $elt );
							break;
						case TABLE_TACHE:
					...
					$doc_model = $document_controller->show( $document_id );
					...
					$document_controller->update( $doc_model );
				}
				else {
		...
		if ( !empty( $documents ) ) {
			foreach ( $documents as $document ) {
				...
				$document_id = $common_data_transfer->transfer_document( $document, $new_element_id, 'document' );
				if ( ( null !== $document_id ) && !is_wp_error( $document_id ) ) {
					$response[ 'transfered' ]++;
		...
		if ( !empty( $duers ) ) {
			foreach ( $duers as $duer ) {
				...
				$document_id = $common_data_transfer->transfer_document( $duer, $new_element_id, 'printed_duer' );
				if ( ( null !== $document_id ) && !is_wp_error( $document_id ) ) {
						$response[ 'transfered' ]++;
		...
		if ( !empty( $sheets ) ) {
			foreach ( $sheets as $sheet ) {
				...
				$document_id = $common_data_transfer->transfer_document( $sheet, $new_element_id, 'printed_sheet' );
				if ( ( null !== $document_id ) && !is_wp_error( $document_id ) ) {
						$response[ 'transfered' ]++;
