		...
		if ( !empty( $eva_danger_categories ) ) {
			...
			foreach ( $eva_danger_categories as $eva_danger_category ) {
				...
				if ( !empty( $wp_danger_category->id ) ) {
					...
					/**	Log	*/
					wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-danger-category', array( 'object_id' => $eva_danger_category->id, 'message' => sprintf( __( 'Danger category %s transfered from evarisk on taxonomy having id. %d', 'digirisk' ), $eva_danger_category->nom, $wp_danger_category->id), ), 0 );
					...
					if ( !empty( $eva_dangers ) ) {
						...
						foreach ( $eva_dangers as $eva_danger ) {
							...
							if ( !empty( $wp_danger->id ) ) {
								...
								$eva_danger_category_transfered++;
								/**	Log	*/
								wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-danger', array( 'object_id' => $eva_danger->id, 'message' => sprintf( __( 'Danger %s transfered from evarisk on taxonomy having id. %d', 'digirisk' ), $eva_danger->nom, $eva_danger->id), ), 0 );
								...
								wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-danger-category', array( 'object_id' => $eva_danger_category->id, 'message' => sprintf( __( 'Error transferring danger %s from evarisk to taxonomy. Error: %s', 'digirisk' ), $eva_danger->nom, json_encode( $wp_danger ) ), ), 2 );
							}
						}
					}
					...
					wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-danger-category', array( 'object_id' => $eva_danger_category->id, 'message' => sprintf( __( 'Error transferring danger category %s from evarisk to taxonomy. Error: %s', 'digirisk' ), $eva_danger_category->nom, json_encode( $wp_danger_category ) ), ), 2 );
				}
			}
		}
		...
		if ( !empty( $eva_vars ) ) {
			...
			foreach ( $eva_vars as $eva_var ) {
				...
				if ( !empty( $wp_var->id ) ) {
					...
					$eva_vars_transfered++;
					/**	Log creation	*/
					wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-evaluation-vars', array( 'object_id' => $eva_var->id, 'message' => sprintf( __( 'Evaluation method variable %s transfered from evarisk on taxonomy having id. %d', 'digirisk' ), $eva_var->nom, $wp_var->id ), ), 0 );
					...
					wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-evaluation-vars', array( 'object_id' => $eva_var->id, 'message' => sprintf( __( 'Error transferring evaluation method variable %s from evarisk to taxonomy. error: %s', 'digirisk' ), $eva_var->nom, json_encode( $wp_var ) ), ), 2 );
				}
			}
		}
		...
		if ( !empty( $eva_methods ) ) {
	        ...
	        AND t1.Status = 'Valid'
				...
				if ( !is_wp_error( $wp_evaluation_method ) ) {
					...
					/**	Log creation	*/
					wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-evaluation-method', array( 'object_id' => $eva_method->id, 'message' => sprintf( __( 'Evaluation method %s transfered from evarisk on taxonomy having id. %d', 'digirisk' ), $eva_method->nom, $wp_evaluation_method->id ), ), 0 );
					...
					wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-evaluation-method', array( 'object_id' => $eva_method->id, 'message' => sprintf( __( 'Error transferring evaluation method %s from evarisk to taxonomy. ERROR: %s', 'digirisk' ), $eva_method->nom, json_encode( $wp_evaluation_method ) ), ), 2 );
				}
			}
		}
		...
		if ( !empty( $eva_recommendation_categories ) ) {
			...
			foreach ( $eva_recommendation_categories as $eva_recommendation_category ) {
				...
				if ( !empty( $wp_category_recommendation->id ) ) {
					...
					/**	Log creation	*/
					wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-recommendation', array( 'object_id' => $eva_recommendation_category->id, 'message' => sprintf( __( 'Recommendation category %s transfered from evarisk on taxonomy having id. %d', 'digirisk' ), $eva_recommendation_category->nom, $wp_category_recommendation->id ), ), 0 );
					...
					if ( !empty( $eva_recommendations ) ) {
						...
						foreach ( $eva_recommendations as $eva_recommendation ) {
							...
							if ( !empty( $wp_recommendation->id ) ) {
								...
								/**	Log creation	*/
								wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-recommendation', array( 'object_id' => $eva_recommendation->id, 'message' => sprintf( __( 'Recommendation %s transfered from evarisk on taxonomy having id. %d', 'digirisk' ), $eva_recommendation->nom, $wp_recommendation->id ), ), 0 );
								...
								/**	Log creation	*/
								wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-recommendation', array( 'object_id' => $eva_recommendation->id, 'message' => sprintf( __( 'Error while transferring recommendation %s from Evarisk to taxonomy. Error: %s', 'digirisk' ), $eva_recommendation->nom, $wp_recommendation ), ), 0 );
							}
						}
					}
				}
				else {
					wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-recommendation-category', array( 'object_id' => $eva_recommendation_category->id, 'message' => sprintf( __( 'Error transferring recommendation category from evarisk to taxonomy. Error: %s', 'digirisk' ), $eva_recommendation_category->nom, $wp_category_recommendation ), ), 2 );
				}
			}
		}
	...
	function transfer_document_model( $current_transfer_state ) {
		...
		foreach ( $model_list as $model ) {
			...
			if ( is_int( $model_id ) ) {
				...
				wpeologs_ctr::log_datas_in_files( 'digirisk-datas-transfert-model', array( 'object_id' => $model->id, 'message' => sprintf( __( 'Error transferring model named %s from evarisk to wordpress media under id. %d', 'digirisk' ), $model->nom, $model_id ), ), 2 );
			}
		}
