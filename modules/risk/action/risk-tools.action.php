...
class Risk_Tools_Action {
	...
	public function callback_digi_risk_preset_reset() {
		...
		delete_option( \eoxia001\Config_Util::$init['digirisk']->setting->key_preset_danger );
		...
		if ( ! empty( $presets_risks ) ) {
			foreach ( $presets_risks as $risk ) {
				...
				if ( 0 === $builded_risk->current_evaluation_id && ( ! isset( $builded_risk->comment ) || empty( $builded_risk->comment ) || ( ( 1 === count( $builded_risk->comment ) ) && ( 0 === $builded_risk->comment[0]->id ) ) ) ) {
					...
					\eoxia001\LOG_Util::log( sprintf( __( 'Le risque %1$d a été mis à la corbeille car rien n\'avait été configuré', 'digirisk' ), $risk_id ), 'digirisk-tools' );
				} else {
					\eoxia001\LOG_Util::log( sprintf( __( 'Le risque %1$d n\'a pas été modifié car il avait déjà été configuré.', 'digirisk' ), $risk_id ), 'digirisk-tools' );
				}
			}
		}
