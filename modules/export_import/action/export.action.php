...
class export_action {
	...
	public function callback_export_data() {
		...
		if ( !empty( $first_element_id ) ) {
			$export_content = export_class::get()->export( $first_element_id, $what_to_export );
			/** Récupération de la définition de l'élément passé en paramètre / Get the definition of given element */
			$element = society_class::get()->show_by_type( $first_element_id );
			...
			foreach ( $group_list as $group ) {
				$export_content = export_class::get()->export( $group->id, $what_to_export );
			}
			$filename = 'global';
		}
		...
		$sub_response = document_class::get()->create_zip( $export_base . '.zip', array( array( 'link' => $json_filename, 'filename' => basename( $json_filename ) ) ), $element, null );
