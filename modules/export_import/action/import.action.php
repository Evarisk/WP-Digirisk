...
class import_action {
	...
	public function callback_import_data() {
		...
		if ( !zip_util::g()->unzip( $zip_file['tmp_name'], $this->destination_directory ) ) {
			wp_send_json_error();
		}
		...
		$file_content = file_get_contents( $this->destination_directory . str_replace( 'zip', 'json', $zip_file['name'] ) );
