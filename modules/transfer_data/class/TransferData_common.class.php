...
class TransferData_common_class extends \eoxia001\Singleton_Util {
	...
	function transfer( $element_type, $element, $element_parent = null ) {
		...
		if ( empty( $digirisk_transfer_options ) || empty( $digirisk_transfer_options[ $element_type ] ) || !in_array( $element->id, $digirisk_transfer_options[ $element_type ]) ) {
			...
			if ( is_int( $element_id ) && ( 0 !== (int)$element_id ) ) {
				/**	Log creation	*/
				\eoxia001\log_class::g()->exec( 'digirisk-datas-transfert-' . $element_type, '', sprintf( __( 'Transfered from evarisk on post having id. %d', 'wp-digi-dtrans-i18n' ), $element_id), array( 'object_id' => $element->id, ), 0 );
				...
				\eoxia001\log_class::g()->exec( 'digirisk-datas-transfert-' . $element_type, '', sprintf( __( 'Error transferring from evarisk to post. error %s', 'wp-digi-dtrans-i18n' ), json_encode( $element_id ) ), array( 'object_id' => $element_type . '-' . $element->id, ), 2 );
			}
		}
	...
	function transfer_surveys( $old_element_id, $old_element_type, $new_element_id ) {
		...
		if ( !empty( $survey_results ) ) {
			foreach ( $survey_results as $original_survey_id => $final_survey ) {
				...
				\eoxia001\log_class::g()->exec( 'digirisk-datas-transfert-survey' , '', __( 'Survey association have been transfered to normal way', 'wp-digi-dtrans-i18n' ), array( 'object_id' => $original_survey_id, ), 0 );
			}
		}
	}
		...
		if ( is_file( $file ) ) {
			...
			if ( $the_file_content !== false ) {
				...
				if ( !empty( $new_element_id ) && ( 'picture' == $main_type ) && ( 'yes' == $document->isMainPicture ) ) {
					...
					\eoxia001\log_class::g()->exec( 'digirisk-datas-transfert-' . $main_type , '', sprintf( __( 'Définition de l\'image principale %2$d de l\'élément %1$d', 'wp-digi-dtrans-i18n' ), $new_element_id, $attach_id ), array( 'object_id' => $document->id, 'document_old_def' => $document ), 0 );
				}
				...
				\eoxia001\log_class::g()->exec( 'digirisk-datas-transfert-' . $main_type , '', sprintf( __( '%1$s #%2$d transférée dans les médias de WordPress vers #%3$d sur l\'élément %4$s ', 'wp-digi-dtrans-i18n' ), $main_type, $document->id, $attach_id, $new_element_id), array( 'object_id' => $document->id, ), 0 );
				...
				\eoxia001\log_class::g()->exec( 'digirisk-datas-transfert-' . $main_type , '', sprintf( __( '%s could not being transfered to wordpress element. Filename: %s. Wordpress element: %d. Evarisk old element: %s', 'wp-digi-dtrans-i18n' ), $main_type, $file, $new_element_id, $old_evarisk_element ), array( 'object_id' => $document->id, ), 2 );
			}
		} else {
			$digirisk_transfert_options[ $document_origin ][ 'nok' ][ $document->id ][ 'file' ] = $file;
			\eoxia001\log_class::g()->exec( 'digirisk-datas-transfert-' . $main_type , '', sprintf( __( '%1$s could not being transfered to wordpress element because it is not a file. Path: %2$s.', 'wp-digi-dtrans-i18n' ), $main_type, $file ), array( 'object_id' => $document->id, ), 2 );
		}
			...
			foreach ( $pictures as $picture ) {
				...
				if ( ( null !== $document_id ) && !is_wp_error( $document_id ) ) {
					if ( 'yes' == $picture->isMainPicture ) {
						...
						\eoxia001\log_class::g()->exec( 'digirisk-datas-transfert-picture' , '', sprintf( __( 'Association de l\'image %2$d à l\'élément %1$d', 'wp-digi-dtrans-i18n' ), $new_element_id, $document_id ), array( 'object_id' => $picture->id, 'document_old_def' => $picture ), 0 );
						...
						$associated_document_list[ '_thumbnail' ] = $document_id;
						\eoxia001\log_class::g()->exec( 'digirisk-datas-transfert-picture' , '', sprintf( __( 'Définition de l\'image principale %2$d de l\'élément %1$d', 'wp-digi-dtrans-i18n' ), $new_element_id, $document_id ), array( 'object_id' => $picture->id, 'document_old_def' => $picture ), 0 );
					}
